// pr-home.jsx — P2: Progress dashboard with rings, heatmap, streak

const { useMemo: useHomeMemo, useState: useHomeState, useRef: useHomeRef, useEffect: useHomeEffect } = React;

function EditableName() {
  const [name, setName] = useHomeState(() => window.LS.get("offsec-py-name", ""));
  const [editing, setEditing] = useHomeState(false);
  const [draft, setDraft] = useHomeState(name);
  const inputRef = useHomeRef(null);

  useHomeEffect(() => { if (editing) inputRef.current?.focus(); }, [editing]);

  function commit() {
    const trimmed = draft.trim();
    if (trimmed) { setName(trimmed); window.LS.set("offsec-py-name", trimmed); }
    setEditing(false);
  }
    ...
    return (
      <input
        ...
        style={{
          ...
          width: Math.max(120, draft.length * 28) + "px",
        }}
...
function ActivityHeatmap({ activity }) {
  // 84 cells = 12 weeks × 7 days (most recent week last)
    ...
    <div>
      ...
      <div className="hm-legend">
        ...
        <div className="hm-legend-cells">
          {swatch.map(s => <span key={s} className={"hm-cell " + s} style={{ width: 9, height: 9 }} />)}
        </div>
        <span>more</span>
      </div>
    </div>
...
function HomeScreen({ completed, onNavigate, capturedFlags }) {
  const allSubs = window.getAllSubs();

  // Streak + activity from localStorage
  const { streak, activity } = useHomeMemo(() => {
    const s = window.LS.get("offsec-py-streak", { count: 0, lastDate: null });
    const a = window.LS.get("offsec-py-activity", Array(84).fill(0));
    return { streak: s.count, activity: a };
  }, [completed]);

  // Per-module progress
  const modules = useHomeMemo(() => window.COURSE.map(mod => {
    ...
    return { ...mod, done, total: subs.length, pct, color: colors[window.COURSE.indexOf(mod) % 4] };
  }), [completed]);

  // Continue lesson = first incomplete
  const continueLesson = useHomeMemo(() => {
    return allSubs.find(({ sub }) => !completed.has(sub.id)) || allSubs[allSubs.length - 1];
  }, [completed]);

  // Overall
  const totalDone = allSubs.filter(({ sub }) => completed.has(sub.id)).length;
  const pctTotal = Math.round((totalDone / allSubs.length) * 100);

  // Use real activity only — empty cells when no activity yet
  const seedActivity = useHomeMemo(() => {
    return activity;
  }, [activity]);
      ...
      <div className="home-inner">
        ...
        <div className="home-grid">
          ...
          <div className="home-card">
            ...
            <ActivityHeatmap activity={seedActivity} />
            <div style={{ marginTop: 16, display: "flex", gap: 14, flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                ...
                <span style={{ fontFamily: "var(--serif)", fontSize: 30, color: "var(--green)", lineHeight: 1 }}>{capturedFlags.size}</span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--fg3)", letterSpacing: ".1em", textTransform: "uppercase" }}>Total flags</span>
                <span style={{ fontFamily: "var(--serif)", fontSize: 30, color: "var(--fg)", lineHeight: 1 }}>{Object.keys(window.LAB_STARTERS).length}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {modules.map(m => (
              <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 16 }}>
                ...
                <button
                  style={{ padding: "4px 10px", border: "1px solid var(--border2)", borderRadius: 5, background: "transparent", color: "var(--fg2)", fontSize: 11.5, fontFamily: "var(--sans)" }}
                  onClick={() => {
                    const first = m.sections[0]?.subs?.[0];
