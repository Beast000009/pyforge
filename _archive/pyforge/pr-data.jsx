// pr-data.jsx — Full course data (72 subsections) from Beast000009/Python-Reviewer
// Auto-converted from courseData.ts + labStarters.ts
...
function getAllSubs() {
  ...
  for (const m of courseData)
    for (const s of m.sections)
      for (const sub of (s.subsections || s.subs || []))
...
// ── Difficulty map for labs ──────────────────────────────────
const LAB_DIFF = {
  "s1-1-2": "easy", "s1-1-5": "easy", "s1-1-8": "easy",
  "s1-1-9": "med",  "s1-2-2": "easy", "s2-3-2": "med",
  "s1-7-4": "hard",
};

// Merge diff into labStarters
const LAB_STARTERS = Object.fromEntries(
  Object.entries(labStarters).map(([k, v]) => [k, { ...v, diff: LAB_DIFF[k] || "med" }])
);



// ── Additional lab starters: Module 2 (Network Scripting) ────
const extraLabs = {
  "s2-4-1": {
    filename: "port_scanner.py",
    instructions: "Build a port scanner that checks if ports 22, 80, 443, and 8080 are open on 127.0.0.1. " +
      "Use MockSocket to simulate connections. Print 'PORT <n>: OPEN' or 'PORT <n>: CLOSED' for each.",
    code: `#!/usr/bin/python3
# Exercise: Port Scanner
# Mock socket for simulation (in real lab, use socket.socket)

OPEN_PORTS = [80, 443]  # Simulate these as open

def scan_port(host, port):
    # In a real scanner: s = socket.socket(); s.settimeout(0.5)
    # try: s.connect((host, port)); return True
    # except: return False
    return port in OPEN_PORTS   # Mock

target = "127.0.0.1"
ports = [22, 80, 443, 8080]

for port in ports:
    # TODO: call scan_port and print result
    pass
`,
    expectedOutput: "PORT 22: CLOSED\nPORT 80: OPEN\nPORT 443: OPEN\nPORT 8080: CLOSED\n",
    flag: "PYTHON(Port_Scanner_Deployed)",
    diff: "med",
    hint: "Use: if scan_port(target, port): print(f'PORT {port}: OPEN') else: print(f'PORT {port}: CLOSED')"
  },

  "s2-5-2": {
    filename: "get_request.py",
    instructions: "Simulate an HTTP GET request. Build the raw HTTP/1.1 GET request string for " +
      "the path '/' on host 'target.local'. Print the full request string.",
    code: `#!/usr/bin/python3
# Exercise: Build an HTTP GET request string
...
request = ""  # Build the request string here
print(request)
`,
    expectedOutput: "GET / HTTP/1.1\r\nHost: target.local\r\nConnection: close\r\n\r\n",
    flag: "PYTHON(HTTP_GET_Mastered)",
    diff: "easy",
    hint: 'request = f"GET {path} HTTP/1.1\\r\\nHost: {host}\\r\\nConnection: close\\r\\n\\r\\n"'
  },

  "s2-7-3": {
    filename: "craft_packet.py",
    instructions: "Craft a custom TCP packet header simulation. Create a dictionary representing " +
      "a TCP packet with src_port=4444, dst_port=80, flags='SYN', and print each field on its own line.",
    code: `#!/usr/bin/python3
# Exercise: Craft a custom TCP packet (Scapy simulation)
# In a real scenario: from scapy.all import IP, TCP, send

def craft_tcp_packet(src_port, dst_port, flags):
    # Return a dict representing the packet
    return {}  # Fill this in

packet = craft_tcp_packet(4444, 80, "SYN")

for key, value in packet.items():
    print(f"{key}: {value}")
`,
    expectedOutput: "src_port: 4444\ndst_port: 80\nflags: SYN\n",
    flag: "PYTHON(Packet_Crafter_Unlocked)",
    diff: "med",
    hint: 'return {"src_port": src_port, "dst_port": dst_port, "flags": flags}'
  },
  ...
  "s3-3-3": {
    filename: "hex_ops.py",
    instructions: "Convert the integer 255 to hex, the hex string '0xff' to an integer, " +
      "and the string 'kali' to its hex representation. Print each result on a new line.",
    code: `#!/usr/bin/python3
# Exercise: Hexadecimal conversions

num = 255
hex_str = '0xff'
word = 'kali'

# 1. Convert num to hex string (use hex())
print("")  # fix me

# 2. Convert hex_str to int (use int() with base 16)
print("")  # fix me

# 3. Convert word to hex (use word.encode().hex())
print("")  # fix me
`,
    expectedOutput: "0xff\n255\n6b616c69\n",
    flag: "PYTHON(Hex_Wizard)",
    diff: "easy",
    hint: "print(hex(num)) | print(int(hex_str, 16)) | print(word.encode().hex())"
  },

  "s3-4-4": {
    filename: "char_convert.py",
    instructions: "Print the ASCII code for 'A', the character for code 97, " +
      "and the ASCII codes for each character in 'root' separated by spaces.",
    code: `#!/usr/bin/python3
# Exercise: Character conversions using ord() and chr()

# 1. Print ASCII code of 'A'
print("")  # fix me

# 2. Print character with ASCII code 97
print("")  # fix me

# 3. Print ASCII codes of 'root' separated by spaces
word = "root"
print("")  # fix me
`,
    expectedOutput: "65\na\n114 111 111 116\n",
    flag: "PYTHON(ASCII_Decoded)",
    diff: "easy",
    hint: "print(ord('A')) | print(chr(97)) | print(' '.join(str(ord(c)) for c in word))"
  },

  "s3-7-3": {
    filename: "json_parse.py",
    instructions: "Parse the JSON string below. Print the 'username', then print each item " +
      "in the 'roles' list on its own line.",
    code: `#!/usr/bin/python3
import json

data = '{"username": "r00t", "uid": 0, "roles": ["admin", "sudo", "docker"]}'
...
`,
    ...
    diff: "easy",
    hint: "parsed = json.loads(data) | print(parsed['username']) | for role in parsed['roles']:"
  },

  "s3-7-4": {
    filename: "sqlite_ops.py",
    instructions: "Use SQLite to create an in-memory database with a 'users' table, " +
      "insert two users, and print all rows sorted by username.",
    code: `#!/usr/bin/python3
import sqlite3
...
# 1. Create table: users(id INTEGER PRIMARY KEY, username TEXT)
cur.execute("")  # fix me

# 2. Insert ('root', ) and ('kali', )  — id is auto
cur.execute("INSERT INTO users (username) VALUES (?)", ("root",))
cur.execute("INSERT INTO users (username) VALUES (?)", ("kali",))

# 3. Select all, ordered by username, print each username
for row in cur.execute(""):  # fix me
    print(row[0])

conn.close()
`,
    expectedOutput: "kali\nroot\n",
    flag: "PYTHON(SQLite_Pwned)",
    diff: "med",
    hint: 'CREATE TABLE users(id INTEGER PRIMARY KEY, username TEXT) | SELECT username FROM users ORDER BY username'
  },
};

// Merge into LAB_STARTERS
Object.assign(LAB_STARTERS, extraLabs);

// ── Alias for app compatibility ───────────────────────────────
const COURSE = courseData;

Object.assign(window, { COURSE, courseData, LAB_STARTERS, labStarters, getAllSubs, findSubById, LS });
