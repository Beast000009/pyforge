...
// Progress insights panel — mastery map, module bars, focus recommendations
// Exports: ProgressPanel2 → window  (distinct name to avoid collision with pr-progress.jsx)

const { useMemo: useProg } = React;

function ProgressPanel2({ onBack }) {
  const { state, derived } = useCourseStore();
  const { moduleMastery, dueCards, overallPct, allCards } = derived;

  const totalLessons = (window.COURSE_DATA || []).reduce(
    (s, m) => s + m.sections.flatMap(sec => sec.subsections).length, 0
  );
  ...
  const strongTopics = [...topicMap].reverse().slice(0, 3);
  ...
  const s = {
    ...
    backBtn: { display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', color: 'var(--text-2)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 500, marginBottom: 32 },
    h1: { fontFamily: '"Instrument Serif", Georgia, serif', fontWeight: 400, fontSize: 36, letterSpacing: '-0.01em', marginBottom: 6 },
    sub: { fontSize: 13, color: 'var(--text-2)', marginBottom: 40, lineHeight: 1.5 },
    ...
    statN: { fontFamily: '"Instrument Serif", Georgia, serif', fontSize: 40, lineHeight: 1, color: 'var(--text)' },
    statL: { fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text-3)', marginTop: 5, letterSpacing: '0.04em' },
    sectionH: { fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 14 },
    card: { background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: '20px 22px', marginBottom: 24 },
  };
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={{ height: 3, background: 'var(--border)', flexShrink: 0 }}>
        <div style={{ height: '100%', background: 'var(--primary)', width: overallPct + '%', transition: 'width 600ms ease' }}/>
      </div>
      ...
      <div style={s.inner}>
        ...
        <div style={s.card}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {moduleMastery.map((mm, i) => {
              ...
              const color = mm.pct > 70 ? '#4ade80' : mm.pct > 35 ? 'var(--primary)' : '#fbbf24';
              return (
                <div key={mm.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 7 }}>
                    <span style={{ fontSize: 13.5, fontWeight: 500, color: 'var(--text)' }}>{mod?.number}. {mod?.title}</span>
        ...
        {topicMap.length > 0 && (
          <div style={s.card}>
              ...
              {topicMap.slice(0, 20).map(({ topic, strength }) => {
                ...
                return (
                  <div key={topic} style={{
                    ...
                    <div style={{ fontSize: 10.5, fontWeight: 500, color: strength > 0.55 ? 'rgba(10,10,10,0.8)' : 'var(--text)', lineHeight: 1.2, marginBottom: 3 }}>
                      {topic.length > 22 ? topic.slice(0,20)+'…' : topic}
                    </div>
                    <div style={{ fontSize: 9.5, fontFamily: 'var(--mono)', color: strength > 0.55 ? 'rgba(10,10,10,0.6)' : 'var(--text-3)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12, fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text-3)' }}>
              <span>weak</span>
        ...
        <div style={s.card}>
          ...
          {weakTopics.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--text-2)' }}>Complete lessons and practice cards to see recommendations here.</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {weakTopics.map(({ topic, strength }, i) => (
                <div key={topic} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13.5, fontWeight: 500, color: 'var(--text)', marginBottom: 5 }}>{topic}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text-3)', whiteSpace: 'nowrap' }}>
        ...
        {/* SRS due cards */}
        {dueCards.length > 0 && (
          <div style={{ background: 'rgba(232,75,34,0.07)', border: '1px solid var(--primary-border)', borderRadius: 8, padding: '16px 20px', marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 4, lineHeight: 1.5 }}>
              Press <kbd style={{ background: 'var(--card2)', border: '1px solid var(--border2)', borderRadius: 3, padding: '1px 5px', fontFamily: 'var(--mono)', fontSize: 11 }}>P</kbd> or click Practice in the sidebar to start your session.
            </p>
          </div>
        )}
      </div>
...
Object.assign(window, { ProgressPanel2 });
